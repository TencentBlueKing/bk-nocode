...
const permission = {
  methods: {
    ...
    hasPermission(reqPermission = [], curPermission = []) {
      const { actions } = this.$store.state.permission.permMeta;
    ...
    applyForPermission(reqPermission, curPermission, resourceData) {
      const data = this.getPermissionData(reqPermission, curPermission, resourceData);
     ...
     * @param {Array} resourceData 资源数据
     */
    getPermissionData(reqPermission = [], curPermission = [], resourceData = {}) {
      const { actions, resources, system } = this.$store.state.permission.permMeta;
      const sysData = system.find(item => item.id === 'bk_nocode');
      const { id: systemId, name: systemName } = sysData;
      const actionsData = this.assembleActionsData(
        reqPermission, curPermission, resourceData,
        actions, resources, systemId, systemName
      );
    ...
    /**
     * 打开权限申请弹窗
     * @param {Object} permissions 无权限请求权限中心链接请求参数数据
     */
