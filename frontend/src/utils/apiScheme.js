...
/**
 * 将接口的scheme协议中的array类型的数据过滤掉并将剩余字段转换为tree，使用场景：节点轮询配置选择字段
 * @param {Array} scheme 接口scheme协议
 * @param {Array} exclude 需要过滤的类型
 */
export function transSchemeToFieldTree(scheme, exclude = []) {
  const treeData = [];
  if (scheme.type === 'object') {
    Object.keys(scheme.properties).forEach((key) => {
      const prop = scheme.properties[key];
      if (exclude.includes(prop.type)) {
        return;
      }
      const field = { id: key, name: key, type: prop.type };
      if (prop.type === 'object') {
        field.children = transSchemeToFieldTree(prop, exclude);
      }
      treeData.push(field);
    });
  }
