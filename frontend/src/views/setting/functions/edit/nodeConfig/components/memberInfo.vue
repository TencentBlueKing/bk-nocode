<template>
  <div class="member-content">
    ...
    <template v-if="recipientItem.key !== 'ORGANIZATION'">
      ...
      <template v-else-if="(recipientItem.key === 'GENERAL' || recipientItem.key === 'CMDB')
      && Array.isArray(recipientItem.value)">
...
<script>
import memberSelect from '../../../../../commonComponent/memberSelect/index.vue';
...
export default {
  ...
  created() {
    ...
    this.recipientKeyList = this.globalChoise.processor_type
      .filter(item => !filterList.some(filterName => filterName === item.typeName));
    // 如果一级字段存在值，则调用二级字段列表
  ...
  methods: {
    ...
    secondListFn(recipientItem) {
      ...
      this.$store.dispatch('setting/getSecondUser', {
    ...
    // 组织架构
    getOrganization() {
      this.$store.dispatch('setting/getTreeInfo').then((res) => {
        // 操作角色组织架构
