<template>
  <div class="bk-trigger-condition">
    <div class="bk-condition-name"><span>触发条件</span></div>
    <div class="bk-condition-content" @click="triggerRules.checkStatus = false">
      <ul class="bk-content-step">
        <li class="bk-content-li" v-for="(item, index) in triggerRules.list" :key="index">
          ...
          <div class="bk-content-type" @click="handleBetween">
            <span>{{ triggerRules.type === 'all' ? '且' : '或' }}</span>
            <i class="bk-icon icon-down-shape"></i>
          </div>
          <!-- 最外层线条 -->
          <div class="bk-item-line"></div>
        ...
        <li class="bk-content-add">
                    <span @click="addGroup">
                        <i class="bk-icon icon-plus-circle mr5"></i><span>添加</span>
                    </span>
          <div class="bk-item-line"></div>
      ...
      <p class="bk-error-info" v-if="triggerRules.checkStatus">请填写完整的触发条件</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeContent(node) {
      ...
      node.conditionList = this.globalChoice.trigger_methods[lowerType];
      // 由于itsm没有 'date'组件类型 所以做个兼容
      if (lowerType === 'date') {
        node.conditionList = this.globalChoice.trigger_methods.datetime;
      }
