  ...
  <div class="bk-api-run">
    <div class="bk-basic-item mt20">
      <div class="bk-service-name">
        <h1><span class="is-outline"></span>{{ "配置" }}</h1>
      </div>
...
export default {
  ...
  methods: {
    ...
    async updateApi() {
      if (this.secondClick || !this.apiDetailInfoCommon.treeDataList
        || !this.apiDetailInfoCommon.responseTreeDataList) {
        return;
      }
      try {
        // 2.重置数据 参数和返回数据统一
      ...
      await this.$store.dispatch('manage/getRemoteApi', params).then(() => {
        this.$bkMessage({
          message: '更新成功',
          theme: 'success',
        });
