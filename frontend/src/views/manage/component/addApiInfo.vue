<template>
  <div class="bk-add-api">
    <bk-form :label-width="200" form-type="vertical" :model="directory.formInfo" :rules="rules" ref="addApiForm">
      <bk-form-item :label="'接口分类'" :required="true" :property="'key'">
        <bk-select
          :disabled="!isAll && !!directory.formInfo.key"
          v-model="directory.formInfo.key"
          :clearable="false"
          searchable
          @selected="changeCode">
          <bk-option v-for="option in treeList.slice(1)" :key="option.id" :id="option.id" :name="option.name">
          </bk-option>
        </bk-select>
      </bk-form-item>
      <template v-if="typeInfo === 'JOIN'">
        <bk-form-item :label="'选择接口'" :required="true" :property="'road'">
          <bk-select
            :disabled="!directory.formInfo.key"
      ...
      <bk-form-item :label="'接口名称'" :required="true" :property="'name'">
        <bk-input maxlength="120" :placeholder="'请输入接口名称'" v-model="directory.formInfo.name"></bk-input>
      </bk-form-item>
      <bk-form-item :label="'负责人：'">
        <member-select v-model="directory.formInfo.ownersInputValue"></member-select>
      </bk-form-item>
      <bk-form-item :label="'函数名称'">
        <!-- eslint-disable vue/camelcase -->
        <bk-input :placeholder="'请输入函数名称'" v-model="directory.formInfo.func_name"></bk-input>
        <!-- eslint-enable -->
      </bk-form-item>
      <bk-form-item :label="'接口路径'">
        <bk-input
          v-model="directory.formInfo.road"
          placeholder="please input path"
          :disabled="
            (directory.formInfo.category === 'component' || directory.formInfo.category === 'buffet_component') &&
            typeInfo !== 'ADD'
          ">
      ...
      <bk-form-item :label="'版本号'">
        <bk-input :placeholder="'请输入版本号'" v-model="directory.formInfo.version"></bk-input>
      </bk-form-item>
      <bk-form-item :label="'API描述'">
        <bk-input :placeholder="'请输入API描述'" :type="'textarea'" :rows="3" v-model="directory.formInfo.desc">
        </bk-input>
      </bk-form-item>
    </bk-form>
    ...
    <div class="mb20">
      <bk-button theme="primary" :title="'确认'" :loading="secondClick" class="mr10" @click="submitAdd">
        {{ '确认' }}
      </bk-button>
      <bk-button theme="default" :title="'取消'" :disabled="secondClick" @click="closeAdd">
        {{ '取消' }}
      </bk-button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    isAll() {
      if (this.firstLevelInfo.code) {
        ...
        this.getChannelPathList(this.firstLevelInfo.code);
        return false;
      }
      return true;
    },
  ...
  methods: {
    ...
    addHandel() {
      ...
      this.$store
        .dispatch('manage/postRemoteApi', params)
        .then(() => {
          this.$bkMessage({
            message: '添加成功',
            theme: 'success',
    ...
    changeCode(id) {
      ...
      this.getChannelPathList(dataItem.code);
    },
    async getChannelPathList(code) {
      ...
      try {
        this.pathLoding = true;
        const res = await  this.$store.dispatch('manage/getComponents', params);
        console.log(res);
