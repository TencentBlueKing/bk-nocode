<template>
  <div class="bk-deal-person" :class="{
    ...
    <p class="bk-error-info" v-if="showError">{{ requiredMsg || '处理人不能为空' }}</p>
  </div>
</template>
<script>
import memberSelect from '../../commonComponent/memberSelect/index.vue';
import SelectTree from '../../commonComponent/selectTree/index.vue';
...
export default {
  ...
  methods: {
    ...
    getRoleGroup() {
      ...
      return this.$store.dispatch('workbench/getUser', {
        is_processor: true,
    ...
    secondListFn(type) {
      ...
      this.isLoading = true;
      this.$store.dispatch('workbench/getSecondUser', params).then((res) => {
        let userList = [];
    ...
    // 组织架构
    getOrganization() {
      this.$store.dispatch('workbench/getTreeInfo').then((res) => {
        // 操作角色组织架构
    ...
    getFrontNodesList() {
      ...
      return this.$store.dispatch('setting/getNodeVars', params).then((res) => {
