  ...
  <div v-if="item.showFeild">
    <bk-form-item :label="item.name" :required="item.validate_type === 'REQUIRE'" :desc="item.tips" desc-type="icon">
      <div class="image-file">
        ...
        <div v-for="(img, index) in imgList" :class="['image-item', { disabled: viewMode }]" :key="index">
...
export default {
  ...
  created() {
    if (!this.item.val || this.item.val === '' || (Array.isArray(this.item.val) && !this.item.val.length)) {
      this.item.val = [];
    }
  },
  methods: {
