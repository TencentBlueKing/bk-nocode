...
class Client(object):
    ...
    def grant_resource_creator_actions(self, bk_token, bk_username, data):
        ...
        ok, message, _data = self._call_esb_api(
            http_post, path, data, bk_token, bk_username, timeout=5
        )
        if not ok:
    ...
    # create_grade_managers
    def grade_managers(self, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/grade_managers/"

        ok, message, _data = self._call_esb_api(
            http_post, path, data, bk_token, bk_username, timeout=15
        )
        if not ok:
            return False, message, _data

        return True, message, _data

    def create_user_group(self, grade_manager_id, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/grade_managers/{grade_manager_id}/groups/".format(
            grade_manager_id=grade_manager_id
        )

        ok, message, data = self._call_esb_api(
            http_post, path, data, bk_token, bk_username, timeout=15
        )
        if not ok:
    ...
    def create_user_group_policies(self, user_group_id, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/groups/{group_id}/policies/".format(
    ...
    def delete_user_group(self, group_id, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/groups/{group_id}/".format(
            group_id=group_id
        )
        ok, message, data = self._call_esb_api(
    ...
    def add_user_group_members(self, group_id, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/groups/{group_id}/members/".format(
            group_id=group_id
        )
    ...
    def delete_user_group_members(self, group_id, bk_token, bk_username, data):
        path = "/api/c/compapi/v2/iam/management/groups/{group_id}/members/".format(
            group_id=group_id,
        )
        ok, message, data = self._call_esb_api(
    ...
    def get_user_group(self, grade_manager_id, bk_token, bk_username):
        path = "/api/c/compapi/v2/iam/management/grade_managers/{grade_manager_id}/groups/".format(
            grade_manager_id=grade_manager_id
        )
        data = {}
        # ok, message, _data = self._call_iam_api(http_get, path, {})
        ok, message, data = self._call_esb_api(
            http_get, path, data, bk_token, bk_username, timeout=15
        )
        if not ok:
            return False, message, ""
    ...
    def get_user_group_members(self, group_id, bk_token, bk_username):
        path = "/api/c/compapi/v2/iam/management/groups/{group_id}/members/".format(
            group_id=group_id
        )
        data = {}
    ...
    #     path = "/api/v1/roles/super_manager/members/"
    #     ok, message, _data = self._call_iam_api(http_get, path, {})
    #     if not ok:
    #         return False, message, ""
    #
