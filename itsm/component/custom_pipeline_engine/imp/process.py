...
class ProcessMixin:
    def beat(self, process_id: int):
        ...
        :type process_id: int
        """
        self.local.process[process_id]["last_heartbeat"] = timezone.now()
    ...
    def wake_up(self, process_id: int):
        ...
        :type process_id: int
        """
        self.local.process[process_id]["asleep"] = False
    ...
    def sleep(self, process_id: int):
        ...
        :type process_id: int
        """
        self.local.process[str(process_id)]["asleep"] = True
    ...
    def suspend(self, process_id: int, by: str):
        ...
        :type by: str
        """
        self.local.process[process_id].update({"suspended": True, "suspended_by": by})
    ...
    def resume(self, process_id: int):
        ...
        :type process_id: int
        """
        self.local.process[process_id].update({"suspended": False, "suspended_by": ""})
    ...
    def batch_resume(self, process_id_list: List[int]):
        ...
        """
        Process.objects.filter(id__in=process_id_list).update(
            suspended=False, suspended_by=""
        )
    ...
    def die(self, process_id: int):
        ...
        :type process_id: int
        """
        self.local.process[process_id].update(
            {
                "dead": True,
            }
        )
    ...
    def get_process_info(self, process_id: int) -> ProcessInfo:
        ...
        :rtype: ProcessInfo
        """
        process = self.local.process[process_id]

        return ProcessInfo(
            process_id=process["id"],
            destination_id=process.get("destination_id", ""),
            root_pipeline_id=process["root_pipeline_id"],
            pipeline_stack=json.loads(process["pipeline_stack"]),
            parent_id=process.get("parent_id", -1),
        )
    ...
    def get_sleep_process_info_with_current_node_id(
        self, node_id: str
        ...
        if len(qs) != 1:
            raise ValueError(
                "found multiple sleep process({}) with current_node_id({})".format(
                    qs, node_id
                )
            )
    ...
    def get_process_id_with_current_node_id(self, node_id: str) -> Optional[str]:
        ...
        if len(qs) != 1:
            raise ValueError(
                "found multiple process({}) with current_node_id({})".format(
                    qs, node_id
                )
            )
    ...
    def child_process_finish(self, parent_id: int, process_id: int) -> bool:
        ...
        row = Process.objects.filter(id=parent_id, ack_num=F("need_ack")).update(
            ack_num=0, need_ack=-1
        )
    ...
    def is_frozen(self, process_id: int) -> bool:
        ...
        :rtype: bool
        """
        process = self.local.process.get(process_id)
        if process is None:
            return False

        return process.get("frozen", False)
    ...
    def fork(
        ...
        if not qs:
            raise Process.DoesNotExist(
                "Process with id({}) does not exist".format(parent_id)
            )
        ...
        qs = Process.objects.filter(parent_id=parent_id, dead=False).only(
            "id", "current_node_id"
        )
    ...
    def join(self, process_id: int, children_id: List[str]):
        ...
        Process.objects.filter(id=process_id).update(
            ack_num=0, need_ack=len(children_id)
        )
