...
class DataMixin(SerializerMixin):
    def _get_data_inputs(self, inputs: dict):
        return {
            k: DataInput(need_render=v["need_render"], value=v["value"])
    ...
    def get_data(self, node_id: str) -> Data:
        ...
        """
        try:
            data = self.local.datas.get(node_id)
        except DBData.DoesNotExist:
            raise exceptions.NotFoundError
        return Data(
            inputs=self._get_data_inputs(codec.data_json_loads(data["inputs"])),
    ...
    def get_data_inputs(self, node_id: str) -> Dict[str, DataInput]:
        ...
        return self._get_data_inputs(codec.data_json_loads(data["inputs"]))
    ...
    def set_data_inputs(self, node_id: str, data: Dict[str, DataInput]):
        ...
        inputs = codec.data_json_dumps(
            {
                k: {"need_render": v.need_render, "value": v.value}
    ...
    def get_execution_data(self, node_id: str) -> ExecutionData:
        ...
        return ExecutionData(
            ...
            outputs=self._deserialize(
                data_model.outputs, data_model.outputs_serializer
            ),
        )
    ...
    def get_execution_data_inputs(self, node_id: str) -> dict:
        ...
        :rtype: dict
        """
        qs = DBExecutionData.objects.filter(node_id=node_id).only(
            "inputs_serializer", "inputs"
        )
    ...
    def get_execution_data_outputs(self, node_id: str) -> dict:
        ...
        :rtype: dict
        """
        db_execution_data = self.local.db_execution_data.get(node_id, None)
        if db_execution_data is None:
            return {}

        return self._deserialize(
            db_execution_data["outputs"], db_execution_data["outputs_serializer"]
        )
    ...
    def set_execution_data(self, node_id: str, data: ExecutionData):
        ...
        db_execution_data = self.local.db_execution_data.get(node_id, None)
        if db_execution_data is not None:

            db_execution_data.update(
                inputs=inputs,
            ...
            self.local.db_execution_data[str(node_id)] = {
                "node_id": node_id,
                "inputs": inputs,
                "inputs_serializer": inputs_serializer,
                "outputs": outputs,
    ...
    def set_execution_data_inputs(self, node_id: str, inputs: dict):
        ...
        db_execution_data = self.local.db_execution_data.get(node_id, None)
        if db_execution_data is not None:
            self.local.db_execution_data.update(
                inputs=inputs, inputs_serializer=inputs_serializer
            )
        else:
            self.local.db_execution_data[str(node_id)] = {
                "node_id": node_id,
    ...
    def set_callback_data(self, node_id: str, version: str, data: dict) -> int:
        ...
        :rtype: int
        """
        data_id = random.randint(1, 10000000)
        self.local.callback_data[data_id] = {
            "id": data_id,
            "node_id": node_id,
            "version": version,
    ...
    def get_callback_data(self, data_id: int) -> CallbackData:
        ...
        :rtype: CallbackData
        """
        data_model = self.local.callback_data[data_id]
        return CallbackData(
            id=data_model["id"],
            node_id=data_model["node_id"],
