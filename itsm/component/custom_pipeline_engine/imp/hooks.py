...
class HooksMixin:
    ...
    def pre_skip_conditional_parallel_gateway(
        self, node_id: str, flow_ids: list, converge_gateway_id: str
    ):
        """
        跳过条件并行网关前执行的钩子
    ...
    def post_skip_conditional_parallel_gateway(
        self, node_id: str, flow_ids: list, converge_gateway_id: str
    ):
        """
        跳过条件并行网关后执行的钩子
    ...
    def post_forced_fail_activity(
        ...
        LogEntry.objects.filter(node_id=node_id, version=old_version).update(
            version=new_version
        )
