...
from itsm.component.custom_pipeline_engine.imp.context import ContextMixin
from itsm.component.custom_pipeline_engine.imp.data import DataMixin
from itsm.component.custom_pipeline_engine.imp.execution_history import (
    ExecutionHistoryMixin,
)
from itsm.component.custom_pipeline_engine.imp.hooks import HooksMixin
from itsm.component.custom_pipeline_engine.imp.node import NodeMixin
from itsm.component.custom_pipeline_engine.imp.plugin_manager import (
    PipelinePluginManagerMixin,
)
from itsm.component.custom_pipeline_engine.imp.process import ProcessMixin
from itsm.component.custom_pipeline_engine.imp.schedule import ScheduleMixin
from itsm.component.custom_pipeline_engine.imp.state import StateMixin
from itsm.component.custom_pipeline_engine.imp.task import TaskMixin
from itsm.component.custom_pipeline_engine.imp.utils import PipelineLocal


class CustomBambooDjangoRuntime(
    TaskMixin,
    ExecutionHistoryMixin,
    ContextMixin,
    DataMixin,
    ...
    def _gen_executable_end_event_node(
        self, event: dict, pipeline: dict, root_id: str, parent_id: str
    ) -> Node:
        return Node(
            node_id=event["id"],
            detail=json.dumps(
            ...
            detail=json.dumps(
                {
                    ...
                    "type": event["type"],
                    "targets": {
                        event["outgoing"]: pipeline["flows"][event["outgoing"]][
                            "target"
                        ]
                    }
    ...
    def _gen_gateway_node(
        self, gateway: dict, pipeline: dict, root_id: str, parent_id: str
    ) -> Node:
        if gateway["type"] != NodeType.ConvergeGateway.value:
            targets = {
                flow_id: pipeline["flows"][flow_id]["target"]
                for flow_id in gateway["outgoing"]
            }
        else:
            targets = {
                gateway["outgoing"]: pipeline["flows"][gateway["outgoing"]]["target"]
            }
        ...
        if gateway["type"] == NodeType.ExclusiveGateway.value:
            ...
            raise ValueError(
                "unsupport gateway type {}: {}".format(gateway["type"], gateway)
            )

        return Node(node_id=gateway["id"], detail=json.dumps(detail))

    def _gen_activity_node(
        self, act: dict, pipeline: dict, root_id: str, parent_id: str
    ) -> Node:
        return Node(
            node_id=act["id"],
            detail=json.dumps(
                {
                    ...
                    "targets": {
                        act["outgoing"]: pipeline["flows"][act["outgoing"]]["target"]
                    },
                    "root_pipeline_id": root_id,
                    "parent_pipeline_id": parent_id,
    ...
    def _gen_subproc_node(
        self, subproc: dict, pipeline: dict, root_id: str, parent_id: str
    ) -> Node:
        return Node(
            node_id=subproc["id"],
            detail=json.dumps(
                {
                    ...
                    "targets": {
                        subproc["outgoing"]: pipeline["flows"][subproc["outgoing"]][
                            "target"
                        ]
                    },
                    "root_pipeline_id": root_id,
    ...
    def _prepare(
        ...
        for key, input_data in pipeline["data"]["inputs"].items():
            ...
            if not source_act:
                context_var_references[key] = Template(
                    input_data["value"]
                ).get_reference()
                ...
                if isinstance(source_act, list):
                    for sa in source_act:
                        node_outputs.setdefault(sa["source_act"], {})[
                            sa["source_key"]
                        ] = key
                else:
        ...
        datas.append(
            Data(
                ...
                inputs=codec.data_json_dumps(
                    {"pre_render_keys": {"need_render": False, "value": []}}
                ),
                outputs={},
            )
        )
        ...
        for act in pipeline["activities"].values():
            if act["type"] == NodeType.ServiceActivity.value:
                # node
                nodes.append(
                    self._gen_activity_node(
                        act=act, pipeline=pipeline, root_id=root_id, parent_id=parent_id
                    )
                )
                # data
                data_inputs, compute_cvs = self._data_inputs_assemble(
                    parent_id, act["id"], act["component"]["inputs"]
                )
                datas.append(
                    Data(
                        node_id=act["id"],
                ...
                context_outputs.append(
                    ContextOutputs(
                        pipeline_id=act["id"],
                        outputs=json.dumps(act["pipeline"]["data"]["outputs"]),
                    )
                )
                ...
                raise ValueError(
                    "unsupport act type {}: {}".format(act["type"], act["id"])
                )

        # process events
        nodes.append(
            self._gen_event_node(
                event=pipeline["start_event"],
                pipeline=pipeline,
                root_id=root_id,
                parent_id=parent_id,
            )
        ...
        if parent_id == root_id:
            context_outputs.append(
                ContextOutputs(
                    pipeline_id=root_id, outputs=json.dumps(pipeline["data"]["outputs"])
                )
            )
        ...
        **options
        ...
        datas.append(
            Data(
                ...
                inputs=codec.data_json_dumps(
                    {
                        k: {"need_render": False, "value": v}
                        for k, v in root_pipeline_data.items()
                    }
                ),
                outputs="{}",
            )
        ...
        with transaction.atomic():
            pid = self.save_process(pipeline_id, queue, priority)
            self.set_state(
                node_id=pipeline_id,
            ...
            self.save_context_values(context_values)
            self.save_context_outputs(context_outputs)
        return pid

    def save_nodes(self, nodes):
        logger.info("logger obj is {}".format(logger))
        local_nodes = {}
        for node in nodes:
            local_nodes[node.node_id] = json.loads(node.detail)
        self.local.nodes = local_nodes
    ...
    def save_datas(self, datas):
        ...
        for data in datas:
            ...
            local_datas[str(data.node_id)] = value
        self.local.datas = local_datas

    def save_context_values(self, context_values):
        local_context_values = {}
        for context_value in context_values:
            value = {
                "key": context_value.key,
                "type": context_value.type,
                "serializer": context_value.serializer,
                "code": context_value.code,
                "value": context_value.value,
                "references": context_value.references,
            }
            local_context_values.setdefault(str(context_value.pipeline_id), []).append(
                value
            )

        self.local.context_values = local_context_values

    def save_context_outputs(self, context_outputs):
        local_context_outputs = {}
        for context_output in context_outputs:
            value = {"outputs": context_output.outputs}
            local_context_outputs[str(context_output.pipeline_id)] = value

        self.local.context_outputs = local_context_outputs

    def save_process(self, pipeline_id, queue, priority=None):
        process_id = random.randint(1, 999999)
        process = {
            str(process_id): {
                "id": process_id,
                "root_pipeline_id": str(pipeline_id),
                "queue": queue,
                "priority": priority,
    ...
    def get_plain_log_for_node(
        self, node_id: str, history_id: int = -1, version: str = None
    ) -> str:
        """
        读取某个节点某一次执行的日志
        ...
        return "\n".join(
            [
                ...
                .filter(node_id=node_id, version=version)
                .only("message")
            ]
        )
